...
export const ja: TranslationStructure = {
    ...
    settingsAccount: {
        ...
        // Push notifications
        pushTitle: 'プッシュ通知',
        pushFooter: 'セッションに操作が必要なとき——質問、権限リクエスト、または完了したタスク——このデバイスに通知を受け取ります。下のリストには、通知を受け取るよう設定されたデバイスが表示されます。',
        pushPermission: '権限',
        pushPermLabelUnavailable: '利用不可',
        pushPermLabelAllowed: '許可済み',
        pushPermLabelDenied: '拒否',
        pushPermLabelNotRequested: '未リクエスト',
        pushPermChecking: 'このデバイスの通知権限を確認しています。',
        pushPermUnsupported: 'iPhoneでHappyを開いて（ホーム画面に追加して）通知をオンにしてください。',
        pushPermGranted: 'このデバイスは通知を受け取ることができます。',
        pushPermCanAsk: 'システムプロンプトはアプリから再び表示できます。',
        pushPermBlocked: 'iOSがプロンプトの表示を停止しました。システム設定を開いて通知を再度有効にしてください。',
        pushEnableTurnOn: '通知をオンにする',
        pushEnableOpenSettings: '通知設定を開く',
        pushEnableUnavailableTitle: '通知は利用できません',
        pushEnableSubUnsupported: '通知はiPhoneとAndroidでのみ利用できます。',
        pushEnableSubCanAsk: 'iOSがまだ許可している場合、システムプロンプトを再表示します。',
        pushEnableSubBlocked: 'iOSがプロンプトを表示しなくなった場合、システム設定を開きます。',
        pushRefreshTitle: 'このデバイスを更新',
        pushRefreshSubDefault: '通知が届かないですか？タップしてこのデバイスを再接続してください。',
        pushRefreshSubCurrent: ({ token }: { token: string }) => `接続済み · ${token}`,
        pushEnabledSuccess: 'このデバイスの通知が有効になりました。',
        pushNotGranted: '通知の権限が付与されませんでした。',
        pushOpenSettingsTitle: '設定を開く',
        pushOpenSettingsBody: 'iOSはプロンプトを表示しなくなったため、Happyが設定を開きました。',
        pushRefreshedSuccess: 'このデバイスが再接続されました。',
        pushNotEnabledYet: 'このデバイスではまだ通知が有効になっていません。',
        pushRequestFailed: '通知権限のリクエストに失敗しました。',
        pushRefreshFailed: 'このデバイスの再接続に失敗しました。',
        pushLoadFailed: '通知設定の読み込みに失敗しました。',
        pushDevicesTitle: ({ count }: { count: number }) => `登録済みデバイス (${count})`,
        pushDevicesFooter: '通知を受け取るよう設定されたデバイスです。別のデバイスをタップして削除してください。',
        pushDevicesEmpty: '登録済みデバイスなし',
        pushDevicesEmptySub: 'このデバイスが登録されると、ここに表示されます。',
        pushDeviceReceiving: '通知を受信中',
        pushDeviceOther: '別のデバイス — タップして削除',
        pushDeviceRegistered: ({ date }: { date: string }) => `${date}に登録`,
        pushDeviceLastSeen: ({ date }: { date: string }) => `最終確認: ${date}`,
        pushDeleteTitle: 'デバイスを削除',
        pushDeleteBody: ({ name }: { name: string }) => `${name}をアカウントから削除しますか？通知の受信が停止します。`,
        pushDeleteFailed: 'デバイスの削除に失敗しました。',
    },
    ...
    terminal: {
        ...
        settingUpAccount: 'アカウントを設定しています…',
        autoCreateFailed: 'アカウント作成のためにMacに接続できませんでした。Macが起動していて同じWi-Fiに接続されていることを確認し、QRコードをもう一度スキャンしてください。',
    },
